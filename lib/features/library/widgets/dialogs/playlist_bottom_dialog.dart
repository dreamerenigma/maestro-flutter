...
void showPlaylistBottomDialog(
    BuildContext context,
    List<Map<String, dynamic>> playlists,
    int index,
    Function(bool) onPrivacyChanged,
    {required VoidCallback onPlaylistDeleted,
    bool showAdditionalOption = false}
  ) {
  ...
  bool isPublic = playlists[index]['isPublic'];
  String playlistId = playlists[index]['id'];

  Future<void> updatePlaylistPrivacy(bool newIsPublic) async {
    try {
      await FirebaseFirestore.instance.collection('Playlists').doc(playlists[index]['id']).update({
        'isPublic': newIsPublic,
      });
      log('Playlist privacy updated to: $newIsPublic');
    } catch (e) {
      log('Error updating playlist privacy: $e');
    }
  }
          ...
          return Column(
            ...
            children: [
                            ...
                            Row(
                              children: [
                                if (!playlist['isPublic'])
                                  Text(' · ', style: TextStyle(fontWeight: FontWeight.w400, fontSize: AppSizes.fontSizeSm, color: AppColors.grey)),
              ...
              _buildSectionOption(
                ...
                () async {
                  bool newIsPublic = !isPublic;
                  await updatePlaylistPrivacy(newIsPublic);

                  onPrivacyChanged(newIsPublic);

                  setState(() {
                    isPublic = newIsPublic;
                  });
                  Navigator.pop(context);
                  Get.snackbar('Playlist Privacy Updated', isPublic ? 'This playlist is now public' : 'This playlist is now private', snackPosition: SnackPosition.TOP);
                },
                icon: isPublic ? Icons.lock : Icons.lock_open_rounded,
              ),
              _buildSectionOption(context, 'Add music', icon: PixelArtIcons.add_box_multiple, () {
                Navigator.of(context).pop();
                Navigator.push(context, createPageRoute(AddMusicScreen()));
              }),
              _buildSectionOption(context, 'Delete', icon: MonoIcons.delete, () {
                Navigator.of(context).pop();
                showDeletePlaylistDialog(context, playlistId, onPlaylistDeleted);
              }),
              if (showAdditionalOption) ...[
                _buildSectionOption(context, 'Copy playlist', svgIcon: SvgPicture.asset(
